...
class ImageService {
    ...
    private buildCloudinaryUrl(
        ...
        return `https://res.cloudinary.com/your-app/image/fetch/w_${options.width},h_${options.height},q_${options.quality},f_${options.format}/${encodeURIComponent(`http://192.168.1.7:8000${imagePath}`)}`;
    }
    ...
    private buildLaravelUrl(imagePath: string): string {
        ...
        return isTauri ? `http://192.168.1.7:8000${imagePath}` : imagePath;
    }
