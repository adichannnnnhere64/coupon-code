...
const ProductCard: React.FC<CouponCardProps> = ({ coupon }) => {
    ...
    return (
        <Link
            to="/product/$id" params={{ id: coupon.id.toString() }} className="group/card relative w-full h-full block"
            preload="intent">
            {/* ✅ Main Card - Tailwind v4 Compatible */}
            <div className={` relative w-full h-full aspect-[4/3] rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl
        transition-all duration-700 ease-out group-hover/card:scale-105 group-hover/card:rotate-1
        group-hover/card:brightness-110 ${gradientClass} ${!coupon.is_available ? 'grayscale opacity-60'
                    : 'animate-pulse-glow'} `}>
                ...
                ">
                    <div className={` relative w-full h-4/5 max-h-48 backdrop-blur-2xl bg-white/10 border border-white/20
                rounded-xl overflow-hidden shadow-lg hover:bg-white/20 transition-all duration-700
                group-hover/card:shadow-emerald-500/25 `}>
                        <OptimizedImage src={imageUrl} alt={`${coupon.operator.name} ${coupon.plan_type.name}`} className={`
                    w-full h-full object-cover group-hover/card:scale-110 group-hover/card:rotate-2 transition-all
                    duration-1000 ease-out `} loading="lazy" />
                    </div>
                </div>
