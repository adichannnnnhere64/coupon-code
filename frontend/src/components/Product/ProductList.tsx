...
const ProductList: React.FC<ProductListProps> = ({ initialPage = 1 }) => {
  const [currentPage, setCurrentPage] = useState(initialPage);
  const [searchQuery, setSearchQuery] = useState('');

  const queryResult = useQuery({
    queryKey: ['coupons', currentPage, searchQuery] as const,
  ...
  const coupons: Coupon[] = paginatedCoupons?.data?.filter(
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center py-8">
        ...
        <p className="text-lg text-gray-600 max-w-3xl mx-auto leading-relaxed">
          Discover the best recharge coupons from top operators.
          Get amazing deals on talktime, data, and combo packs.
        </p>
      </div>
      ...
      {(!isLoading && !isError) && filteredCoupons.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="text-2xl font-semibold text-gray-900 mb-2">No coupons found</h3>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pt-8 border-t">
          <div className="text-sm text-gray-600 font-medium">
            Showing{' '}
            <span className="font-semibold">{currentItems}</span> to{' '}
            <span className="font-semibold">{Math.min(lastItem, totalItems)}</span>{' '}
            of <span className="font-semibold">{totalItems}</span> coupons
          </div>

          <div className="join shadow-lg" role="navigation" aria-label="Pagination">
            ...
            {(() => {
              ...
              if (startPage > 1) {
                pageButtons.push(<PaginationButton key={1} page={1}>1</PaginationButton>);
                if (startPage > 2) {
                  pageButtons.push(
                    <span key="ellipsis1" className="join-item btn btn-disabled btn-sm">...</span>
                  );
                }
              }
              ...
              if (endPage < totalPages) {
                if (endPage < totalPages - 1) {
                  pageButtons.push(
                    <span key="ellipsis2" className="join-item btn btn-disabled btn-sm">...</span>
                  );
                }
                pageButtons.push(
            ...
            <PaginationButton page={totalPages} disabled={currentPage === totalPages}>
              Last »
            </PaginationButton>
          </div>
        </div>
